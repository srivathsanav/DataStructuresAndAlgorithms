...
public class NArray {
    ...
    public static ArrayList<Integer> repeatedNumber(final List<Integer> a) {
        ...
        System.out.println(    2000000000L + (long)1000000000 * 1000000000);
        ArrayList<Integer> result = new ArrayList<Integer>();
        if (a.size() == 0) {
            return  result;
        }
        long correctSum = (long) a.size() * (a.size()+1) /2 ;
        long missingSum = 0L;
        for (int i =0 ; i < a.size(); i++) {
            missingSum = missingSum + a.get(i);
        }
        long aMinusB =   missingSum - correctSum;

        long correctSquare =   (long)  a.size() * (a.size() +1);
        correctSquare = correctSquare * (2 * a.size() + 1) / 6;

        long missingSquare = 0L;

        for (int i = 0 ; i < a.size(); i++) {
            missingSquare  +=  ((long) a.get(i) * a.get(i));
        }

        long aSquareMinusBSquare = missingSquare - correctSquare;
        long aPlusB = aSquareMinusBSquare / aMinusB;
        long duplicate =  (aPlusB + aMinusB) /2 ;
        long missingNumber = aPlusB - duplicate;
